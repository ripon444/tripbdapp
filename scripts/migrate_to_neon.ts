...
async function migrate() {
  console.log('Connecting to Neon PostgreSQL for TripBD Migration...');
  ...
  console.log('Connected successfully. Beginning migration transaction...');
  ...
  try {
    ...
    console.log('All 28 tables successfully created. Beginning seed data insertion...');
    ...
    for (const [id, name, slug, desc, icon, img, status, order] of categories) {
      await client.query(`
        INSERT INTO service_categories (id, name, slug, description, icon, image, status, sort_order)
        VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
        ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name, description = EXCLUDED.description, icon = EXCLUDED.icon;
      `, [id, name, slug, desc, icon, img, status, order]);
    }
    await client.query(`SELECT setval('service_categories_id_seq', (SELECT MAX(id) FROM service_categories));`);
    console.log('Seeded service_categories.');
    ...
    for (const [id, catId, name, slug, desc, icon, pass, load, base, km, hr, min, status] of vehicleTypes) {
      await client.query(`
        INSERT INTO vehicle_types (id, service_category_id, name, slug, description, icon, passenger_capacity, load_capacity, base_fare, per_km_rate, per_hour_rate, minimum_fare, status)
      ...
      `, [id, catId, name, slug, desc, icon, pass, load, base, km, hr, min, status]);
    }
    await client.query(`SELECT setval('vehicle_types_id_seq', (SELECT MAX(id) FROM vehicle_types));`);
    console.log('Seeded vehicle_types.');

    // SEED: locations (8 Divisions + 64 Districts)
    const locations = [
    ...
    for (const [id, name, slug, type, dist, div, lat, lng, status] of locations) {
      await client.query(`
        INSERT INTO locations (id, name, slug, type, district, division, latitude, longitude, status)
        VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
        ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name, latitude = EXCLUDED.latitude, longitude = EXCLUDED.longitude;
      `, [id, name, slug, type, dist, div, lat, lng, status]);
    }
    await client.query(`SELECT setval('locations_id_seq', (SELECT MAX(id) FROM locations));`);
    console.log('Seeded locations (8 divisions + 64 districts).');

    // SEED: users
    // Valid Bcrypt Hash for 'password123': $2y$12$GZKWOlA.0hefSC6.fUyc4eiJoe/g/dAnGk935FRNsBv3bgiwH0fGm
    const bcryptPass = '$2y$12$GZKWOlA.0hefSC6.fUyc4eiJoe/g/dAnGk935FRNsBv3bgiwH0fGm';
    const users = [
      [1, 'TripBD System Admin', '01700000000', 'admin@tripbd.com', bcryptPass, 'admin', 'active'],
      [2, 'Tanvir Hasan', '01711111111', 'tanvir@gmail.com', bcryptPass, 'customer', 'active'],
      [3, 'Md. Rafiqul Islam', '01822222222', 'rafiq.driver@gmail.com', bcryptPass, 'driver', 'active'],
      [4, 'Jalal Ahmed (Ambulance)', '01933333333', 'jalal.ambulance@gmail.com', bcryptPass, 'driver', 'active']
    ];

    for (const [id, name, phone, email, pass, role, status] of users) {
      await client.query(`
        INSERT INTO users (id, name, phone, email, password, role, status, phone_verified_at)
        VALUES ($1, $2, $3, $4, $5, $6, $7, NOW())
        ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name, password = EXCLUDED.password, role = EXCLUDED.role, status = EXCLUDED.status;
      `, [id, name, phone, email, pass, role, status]);
    }
    ...
    await client.query(`
      INSERT INTO customer_profiles (id, user_id, address, city, district, country, date_of_birth)
      VALUES (1, 2, 'House 42, Road 11, Banani', 'Dhaka', 'Dhaka', 'Bangladesh', '1992-05-14')
      ON CONFLICT (id) DO UPDATE SET address = EXCLUDED.address, district = EXCLUDED.district;
    `);
    await client.query(`SELECT setval('customer_profiles_id_seq', (SELECT MAX(id) FROM customer_profiles));`);
    console.log('Seeded customer_profiles.');

    // SEED: driver_profiles
    await client.query(`
      INSERT INTO driver_profiles (id, user_id, address, city, district, nid_number, license_number, license_expiry, verification_status, online_status, rating, total_trips)
      VALUES 
    ...
    `);
    await client.query(`SELECT setval('driver_profiles_id_seq', (SELECT MAX(id) FROM driver_profiles));`);
    console.log('Seeded driver_profiles.');

    // SEED: wallets
    await client.query(`
      INSERT INTO wallets (id, driver_id, balance)
      VALUES 
      (1, 3, 4500.00),
      (2, 4, 8200.00)
      ON CONFLICT (id) DO UPDATE SET balance = EXCLUDED.balance;
    `);
    await client.query(`SELECT setval('wallets_id_seq', (SELECT MAX(id) FROM wallets));`);
    console.log('Seeded wallets.');

    // SEED: vehicles
    await client.query(`
      INSERT INTO vehicles (id, driver_id, vehicle_type_id, vehicle_number, brand, model, year, color, passenger_capacity, load_capacity, registration_number, verification_status, status)
      VALUES
    ...
    `);
    await client.query(`SELECT setval('vehicles_id_seq', (SELECT MAX(id) FROM vehicles));`);
    ...
    console.log('\n--- VERIFICATION STATS ---');
    const tables = [
      'users', 'customer_profiles', 'driver_profiles', 'service_categories', 
      'vehicle_types', 'locations', 'vehicles', 'wallets', 'system_settings'
    ];
    ...
    const adminUser = await client.query(`SELECT id, name, email, phone, role, status FROM users WHERE email = 'admin@tripbd.com';`);
    console.log('\nAdmin User in Neon PostgreSQL:', adminUser.rows[0]);
